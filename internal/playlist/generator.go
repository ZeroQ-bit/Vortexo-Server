...
func (pg *PlaylistGenerator) GenerateMoviePlaylist(ctx context.Context) ([]SimplePlaylistEntry, error) {
	...
	contentFilters := playlistContentFilters(pg.cfg)
	discoveryFilters := playlistMovieDiscoveryFilters(pg.cfg)
	...
	for page := 1; page <= pg.cfg.TotalPages; page++ {
		movies, err := pg.tmdb.DiscoverMoviesWithFilters(ctx, page, discoveryFilters)
		if err != nil {
		...
		for _, movie := range movies {
			if allowed, _ := services.MovieAllowedByContentFilters(movie, contentFilters); !allowed {
				continue
			}
