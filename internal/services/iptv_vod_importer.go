	...
	for _, xs := range cfg.XtreamSources {
		...
		if err == nil {
			...
			for i, it := range items {
				...
				if importErr != nil {
					if isContentFilterBlock(importErr) {
						summary.Skipped++
					} else {
...
// ErrBlockedContentFilter is returned when an import violates user content filters.
var ErrBlockedContentFilter = errors.New("blocked_content_filter")

func isContentFilterBlock(err error) bool {
	return errors.Is(err, ErrBlockedBollywood) || errors.Is(err, ErrBlockedContentFilter)
}

func contentFiltersFromImportSettings(cfg *isettings.Settings) ContentFilterOptions {
	if cfg == nil {
...
func importMovieBasic(ctx context.Context, store *database.MovieStore, it vodItem) error {
	...
	if existing, errGet := store.GetByTMDBID(ctx, m.TMDBID); errGet == nil && existing != nil {
		...
		if existing.QualityProfile == "" {
			existing.QualityProfile = m.QualityProfile
		}
...
func importSeriesBasic(ctx context.Context, store *database.SeriesStore, it vodItem) error {
	...
	if existing, errGet := store.GetByTMDBID(ctx, s.TMDBID); errGet == nil && existing != nil {
		...
		existing.Monitored = true
		if existing.QualityProfile == "" {
			existing.QualityProfile = s.QualityProfile
		}
		return store.Update(ctx, existing)
	}
...
func importMovie(ctx context.Context, cfg *isettings.Settings, tmdb *TMDBClient, store *database.MovieStore, it vodItem) error {
	...
	if allowed, reason := MovieAllowedByContentFilters(m, contentFiltersFromImportSettings(cfg)); !allowed {
...
func importSeries(ctx context.Context, cfg *isettings.Settings, tmdb *TMDBClient, store *database.SeriesStore, it vodItem) error {
	...
	if allowed, reason := SeriesAllowedByContentFilters(s, contentFiltersFromImportSettings(cfg)); !allowed {
		if reason == "India-origin media" {
			return ErrBlockedBollywood
		}
		return fmt.Errorf("%w: %s", ErrBlockedContentFilter, reason)
...
func CleanupIPTVVOD(ctx context.Context, cfg *isettings.Settings, movieStore *database.MovieStore, seriesStore *database.SeriesStore) error {
	...
	for _, s := range cfg.XtreamSources {
		if s.Enabled {
			active[s.Name] = true
		}
	}
	...
	if err2 == nil {
		...
		for rows2.Next() {
			var id int64
			var metadataJSON []byte
			if err := rows2.Scan(&id, &metadataJSON); err != nil {
				continue
			}
			meta := models.Metadata{}
			_ = json.Unmarshal(metadataJSON, &meta)
