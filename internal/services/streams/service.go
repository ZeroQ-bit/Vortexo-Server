...
func (s *StreamService) ShouldExcludeByQualityType(streamTitle, resolution, hdrType string, excludedQualities string) bool {
	excluded, reason := QualityTypeExcluded(streamTitle, resolution, hdrType, excludedQualities)
	if excluded {
		s.logger.Debug("Stream excluded by quality type", "stream", streamTitle, "reason", reason)
	}
	return excluded
}

// QualityTypeExcluded checks quality-type exclusions without requiring a StreamService.
func QualityTypeExcluded(streamTitle, resolution, hdrType string, excludedQualities string) (bool, string) {
	...
	for _, excl := range exclusions {
		...
		case "hdr":
			// Any HDR-bearing stream, including DV+HDR hybrids.
			if hdrUpper == "HDR" || hdrUpper == "HDR10" || hdrUpper == "HDR10+" ||
				hdrUpper == "DV+HDR" ||
				strings.Contains(titleUpper, "HDR10+") || strings.Contains(titleUpper, "HDR10") ||
				hasQualityToken(titleUpper, "HDR") {
				return true, "HDR"
			}
		case "dv":
			// Dolby Vision only (not DV+HDR)
			isDolbyVision := hdrUpper == "DV" || hdrUpper == "DOLBY VISION" ||
				strings.Contains(titleUpper, "DOLBY.VISION") ||
				strings.Contains(titleUpper, "DOLBY VISION") ||
				strings.Contains(titleUpper, "DOLBYVISION") ||
				hasQualityToken(titleUpper, "DV") ||
				hasQualityToken(titleUpper, "DOVI")
			isHybrid := hdrUpper == "DV+HDR" ||
				strings.Contains(titleUpper, "DV.HDR") ||
				strings.Contains(titleUpper, "DVHDR") ||
				strings.Contains(titleUpper, "DV HDR")
			if isDolbyVision && !isHybrid {
