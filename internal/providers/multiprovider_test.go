...
func TestBuildRuntimeAddonsDisablesConfiguredProviderAddons(t *testing.T) {
	addons := []StremioAddon{
	...
	if len(runtimeAddons) != 0 {
		t.Fatalf("expected provider addons to be disabled, got %#v", runtimeAddons)
	}
}

func TestBuildRuntimeAddonsDoesNotBootstrapDefaultsForRealDebrid(t *testing.T) {
	runtimeAddons := BuildRuntimeAddons(nil, true, "rd-token", true, "")

	if len(runtimeAddons) != 0 {
		t.Fatalf("expected no default addons, got %#v", runtimeAddons)
	}
}
